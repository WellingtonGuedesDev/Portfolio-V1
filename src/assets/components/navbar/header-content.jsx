...
import { leftAnimation, bottomAnimation } from '../../utils/animations';
...
const Header = () => {
    ...
    const subtitleAnimationRef = useRef(null)
    const arrowAnimationRef = useRef(null)
    const atualmenteAnimation = useRef(null)
    const embreveAnimation = useRef(null)
    const passadoAnimation = useRef(null)

    useEffect(() => {
        leftAnimation(nameAnimationRef, 0.5)
        leftAnimation(arrowAnimationRef, 0.5)
        leftAnimation(titleAnimationRef, 1)
        leftAnimation(subtitleAnimationRef, 1.5)

        bottomAnimation(atualmenteAnimation, 0.5)
        bottomAnimation(embreveAnimation, 1)
        bottomAnimation(passadoAnimation, 1.5)
}, [])
    ...
    <>
        <section style={{ height: `calc(100vh - 110px)` }} className="max-w-[1536px] mx-auto flex flex-col px-4 py-14 md:justify-normal md:py-8 md:relative md:px-0">
            <div className=' flex flex-col gap-4 md:w-1/2 md:mb-8 md:mt-14'>
                <h1 ref={nameAnimationRef} className={`flex justify-center text-3xl font-bold antialiased md:text-5xl md:justify-normal md:whitespace-nowrap text-base/5 text-[${currentColor.textTransparent}]`}>Hello, i'm Wellington</h1>
                <h1 ref={titleAnimationRef} className='flex justify-center text-3xl font-bold antialiased md:text-8xl md:justify-normal md:whitespace-nowrap'>Nice To see you</h1>
                <div className="flex justify-center text-center md:mb-8 md:mt-4">
                    <span ref={subtitleAnimationRef} className=' md:text-left text-base/5'>Olá, eu sou welligton, desenvolvedor especializado em<strong className={`text-[${currentColor.primary}] font-semibold`}>react</strong>, <strong className={`text-[${currentColor.primary}] font-semibold`}>nodejs</strong>.Transformo ideias em soluções criativas e eficientes. por tecnologia e soluções inovadoras. Com expertise em [suas principais habilidades, ex: desenvolvimento web, mobile, backend, etc.], estou sempre em busca de novos desafios para transformar ideias em realidade. Explore meus projetos e veja como posso contribuir para o sucesso do seu próximo projeto!</span>
                </div>
                ...
                <div className='flex justify-center text-3xl mt-8 animate-bounce z-0 md:absolute md:top-0 md:transform md:translate-y-1/2 '>
                    <a href="#projetos">
                        <RxDoubleArrowDown ref={arrowAnimationRef}/>
                    </a>
                </div>
            ...
            <div className='flex flex-col mt-12 gap-8 md:flex-row md:mt-12 md:justify-start md:gap-[10rem] md:mt-[6rem]'>
                <div ref={atualmenteAnimation} className='md:w-[250px]'>
                ...
                <div ref={embreveAnimation} className='md:w-[250px]'>
                ...
                <div ref={passadoAnimation} className='md:w-[250px]'>
